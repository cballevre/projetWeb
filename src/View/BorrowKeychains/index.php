            ...
            <table class="table table-striped">
                ...
                <?php foreach($borrowKeychains as $borrowKeychain): ?>
                    <tr>
                        ...
                        <td><?php $user = $borrowKeychain->user();
                            echo $user->getSurname() . " " . $user->getName(
                                ); ?></td>
                        <td>Trousseau
                            n°<?php echo $borrowKeychain->getIdKeychain(
                            ); ?></td>
                        <td><?php echo $borrowKeychain->getDateRetour()->format(
                                'Y-m-d H:i:s'
                            ); ?></td>
                        <td>
                            <button type="button"
                                    class="btn btn-link btn-sm btn-prolonger"
...
<div class="modal fade" id="modal-prolonger" tabindex="-1" role="dialog"
     aria-hidden="true">
            ...
            <form id="modal-prolonger-form" action="" method="post">
                <div class="modal-body">
                    <div class="form-group row">
                        <label class="col-sm-2 form-control-label text-xs-right">Date
                            de retour : </label>
                        <div class="col-sm-10">
        ...
        $(".btn-prolonger").click(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#modal-prolonger-form').attr('action', "<?php echo WEBROOT ?>?controller=<?php echo $this->getName(
                        ) ?>&action=update&id=" + id);
